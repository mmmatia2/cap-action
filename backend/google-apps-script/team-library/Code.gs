...
function doPost(e) {
  try {
    ...
    if (action === "listSessions") return listSessions(e, body);
    if (action === "getSession") return getSession(e, body);
...
function listSessions(e, body) {
  ...
  const limit = Number((body && body.limit) || (e.parameter && e.parameter.limit) || 50);
...
function getSession(e, body) {
  const sessionId = String((body && body.sessionId) || (e.parameter && e.parameter.sessionId) || "").trim();
  if (!sessionId) return jsonResponse({ ok: false, errorCode: "SESSION_ID_REQUIRED" });
