...
3. For team auth in legacy adapter mode, the app defaults to extension bridge auth (`Extension Auth (Legacy Adapter Default)`).
4. Editor-auth mode is available only as explicit transitional testing (`Editor Auth (Legacy Transitional)`), with manual temporary token input.
5. The app calls the Apps Script web app over `fetch`.
6. Uploads originate from the extension background, not from the web app.

## Auth authority

- Current default auth authority for legacy team-adapter reads: extension background (via bridge)
- Target OSS-default auth authority (not implemented yet): web app owned sign-in/session for browser-agnostic team mode.
- Current default token acquisition path (legacy adapter mode):
  ...
  - app sends the acquired token to backend read actions via POST body
- Transitional editor-auth path (explicit opt-in only):
  - operator manually pastes temporary token into app Team source
  - token is kept in-memory for the current tab only (not persisted in `localStorage`)
  - app sends token in POST body for backend read actions
- Retained compatibility outside the current legacy-adapter default path:
  - the Apps Script backend still supports Google session fallback when no access token is present
  - this is retained backend compatibility, not the canonical app auth path for normal operator use
...
### Request: `REQUEST_TEAM_AUTH` (legacy fallback mode)
...
### Response: `TEAM_AUTH_RESPONSE` (legacy fallback mode)
...
- The app now uses POST body token transport for `listSessions`/`getSession` in both auth-owner modes.
- `Extension Auth (Legacy Adapter Default)` still uses bridge token acquisition; only request transport changed.
- Editor-auth transitional token is in-memory only for the current tab session.
- Query-param token transport remains in backend code only as compatibility fallback for non-updated callers.
...
- App canonical path (both auth-owner modes):
  - `POST ?action=listSessions&protocolVersion=1.0.0`
  - Request uses `Content-Type: text/plain;charset=utf-8` (simple CORS request for hosted browser viability)
  - Body is JSON text including `accessToken` and `limit`
- Backend compatibility path (not canonical app behavior):
...
- App canonical path (both auth-owner modes):
  - `POST ?action=getSession&protocolVersion=1.0.0`
  - Request uses `Content-Type: text/plain;charset=utf-8` (simple CORS request for hosted browser viability)
  - Body is JSON text including `accessToken` and `sessionId`
- Backend compatibility path (not canonical app behavior):
...
- Canonical callers should use only:
  - `REQUEST_SESSIONS`
  - `REQUEST_TEAM_AUTH`
- The app now exposes editor-auth token input for Team Library reads in `Editor Auth (Legacy Transitional)` mode (manual/in-memory transition testing only).
